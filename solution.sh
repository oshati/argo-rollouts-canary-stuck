...
kubectl argo rollouts status like-service -n bleater 2>/dev/null || true
kubectl get analysisrun -n bleater -l rollout=like-service 2>/dev/null || true
  ...
  - name: error-rate
    ...
    provider:
      prometheus:
        ...
        query: |
          sum(rate(http_requests_total{service="like-service",code=~"5.."}[2m]))
          /
          sum(rate(http_requests_total{service="like-service"}[2m]))
...
kubectl patch rollout like-service -n bleater --type merge -p '
{
  "spec": {
...
kubectl argo rollouts abort like-service -n bleater 2>/dev/null || true
sleep 10

echo "[solution] Step 5: Retrying the rollout..."

# Retry the rollout with the fixed template
kubectl argo rollouts retry rollout like-service -n bleater 2>/dev/null || true
sleep 5
...
for i in $(seq 1 5); do
  sleep 30
  STATUS=$(kubectl argo rollouts status like-service -n bleater 2>/dev/null | head -1 || echo "unknown")
  echo "[solution] Rollout status: ${STATUS}"

  if echo "${STATUS}" | grep -qi "paused"; then
    echo "[solution] Promoting past pause..."
    kubectl argo rollouts promote like-service -n bleater 2>/dev/null || true
...
kubectl argo rollouts status like-service -n bleater 2>/dev/null || true
kubectl get analysisrun -n bleater -l rollout=like-service --sort-by='.metadata.creationTimestamp' 2>/dev/null | tail -3
...
if [ -n "${PROM_POD}" ]; then
  QUERY_RESULT=$(kubectl exec -n monitoring "${PROM_POD}" -- wget -qO- \
    'http://localhost:9090/api/v1/query?query=sum(rate(http_requests_total{service="like-service"}[2m]))' 2>/dev/null || echo "query failed")
